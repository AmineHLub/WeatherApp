...
export function WeatherHero({ location, current, today }: Props) {
  const label = conditionLabel[current.condition_code] ?? 'Unknown'
  ...
  return (
    <div className="flex items-center justify-between px-7 py-6 bg-secondary-bg rounded-2xl min-h-[160px] font-syne">
      <div className="flex flex-col gap-4">
        ...
        <div className="text-[48px] md:text-[64px] font-[700] text-text leading-[1] mt-1">{Math.round(current.temperature)}°</div>
      </div>
